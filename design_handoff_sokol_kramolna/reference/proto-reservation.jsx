// proto-reservation.jsx — sdílený rezervační wizard pro Tenis i Posilovnu.
// 3 jednoduché kroky: 1) výběr termínu (hodinové sloty) → 2) údaje → 3) potvrzení,
// následuje úspěšná obrazovka s potvrzovacím kódem „odeslaným na e-mail“.

const { useState, useMemo, useRef } = React;

// ---- datum / čas utility ----
const DOW = ['Ne', 'Po', 'Út', 'St', 'Čt', 'Pá', 'So'];
const NOW = new Date(2026, 4, 30, 14, 20); // pevné „teď“ pro prototyp: So 30. 5. 2026, 14:20
const DAY_MS = 86400000;
const epochDay = (d) => Math.floor(d.getTime() / DAY_MS);
const fmtDM = (d) => `${d.getDate()}. ${d.getMonth() + 1}.`;
const fmtDMY = (d) => `${d.getDate()}. ${d.getMonth() + 1}. ${d.getFullYear()}`;

// deterministická pseudonáhoda podle absolutního dne + hodiny
function seed(dayNo, h) {
...
// startovní den okna: dnešek + offset týdnů (rezervujeme dopředu, ne do minulosti)
...
const MODES = {
  ...
  gym: {
    ...
    unit: 'vstupu',
    priceFor: () => 100, // jednorázový vstup za hodinový blok
    capacity: 15,
  },
};

function ReservationFlow({ mode, onGoOverview }) {
  const cfg = MODES[mode];
  const [step, setStep] = useState(1);
  const [weekOff, setWeekOff] = useState(0);
  // výběr držíme v JEDNOM stavu, ať je update atomický i při rychlém dvojkliku
  const [sel, setSel] = useState({ dayNo: null, slots: [] });
  const dayNo = sel.dayNo;
  const slots = sel.slots;
  const [form, setForm] = useState({ name: '', email: '', phone: '', note: '', agree: false });
  const [touched, setTouched] = useState(false);
  const [code, setCode] = useState('');
  const scrollRef = useRef(null);

  const week = useMemo(() => {
    const start = weekStart(weekOff);
    return HOURS && Array.from({ length: 7 }, (_, i) => {
  ...
  // status slotu: 'past' | 'busy'/'full' | 'free' ; pro gym i remaining
  function slotInfo(date, h) {
  ...
  function clickSlot(date, h, st) {
    ...
    setSel((prev) => {
      ...
      if (cur.includes(h)) {
        // odebrat jen krajní hodinu (zachovat souvislost bloku)
      ...
      return { dayNo: dn, slots: [h] }; // nesouvislé → nový blok
    });
  }
  ...
  function reset() {
    setStep(1); setSel({ dayNo: null, slots: [] });
    setForm({ name: '', email: '', phone: '', note: '', agree: false });
    setTouched(false); setCode('');
  }
        ...
        <div className="skp-success-card">
          ...
          <div className="skp-success-body">
            <div className="skp-code">
              ...
              <span className="mailto"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m3 7 9 6 9-6"/></svg> {form.email}</span>
            </div>
  ...
  const summary = (
    <aside className="skp-summary">
      ...
      <div className="skp-btn-row">
        ...
        {step === 2 && (
          <button className="skp-btn-primary" disabled={!formOk} onClick={() => goStep(3)}>
            Pokračovat <Icon.arrowR size={18} />
          </button>
        )}
        {step === 3 && (
  ...
  const stepper = (
    <div className="skp-stepper">
      {[['1', 'Termín'], ['2', 'Vaše údaje'], ['3', 'Potvrzení']].map(([n, label], i) => {
        const idx = i + 1;
  ...
  if (step === 1) {
    mainContent = (
      <div className="sk-cal-panel skp-cal" style={{ flex: 1 }}>
        <div className="skp-weeknav">
          ...
          <button className="sk-cal-pill icon" onClick={() => setWeekOff((w) => Math.max(0, w - 1))} disabled={weekOff === 0} style={weekOff === 0 ? { opacity: 0.4, cursor: 'not-allowed' } : null}><Icon.chev dir="left" /></button>
          <button className="sk-cal-pill" onClick={() => setWeekOff(0)}>Tento týden</button>
          <button className="sk-cal-pill icon" onClick={() => setWeekOff((w) => Math.min(3, w + 1))}><Icon.chev /></button>
        </div>
          ...
          {HOURS.map((h) => (
            <React.Fragment key={h}>
              ...
              {week.map((d) => {
                ...
                return (
                  <div key={d.getTime() + '-' + h} className={cls}
    ...
    const showErr = (ok) => touched && !ok ? ' err' : '';
    mainContent = (
      <div className="skp-form-card">
        ...
        <div className="skp-form-grid">
          ...
          <div className="skp-field full">
            ...
            <textarea className="skp-input" value={form.note} placeholder={mode === 'tenis' ? 'Např. půjčení vybavení, počet hráčů…' : 'Např. první návštěva, potřebuji instruktora…'}
    ...
    // označit „touched“ při pokusu pokračovat řešíme v summary tlačítku – nastav touched při blur kdykoli
  } else if (step === 3) {
    mainContent = (
  ...
  // při kroku 2 nastavíme touched, aby validace tlačítka ukazovala chyby
  const onMainClick = step === 2 ? () => setTouched(true) : undefined;

  return (
    <div ref={scrollRef} className="skp-scroll" style={{ paddingRight: 4 }}>
      <div style={{ marginBottom: 20 }}>{stepper}</div>
      <div className="skp-resv">
        <div className="skp-resv-main" onClick={onMainClick}>{mainContent}</div>
        {summary}
      </div>
    </div>
  );
...
window.ReservationFlow = ReservationFlow;
